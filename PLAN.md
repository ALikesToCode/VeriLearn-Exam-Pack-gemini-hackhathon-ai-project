...
### Stretch
- Deep Research blueprint report (syllabus + past papers) with citations.
- Live oral-viva mode via Live API.
- "Assist mode" computer-use for resource collection.
...
2) Gemini video understanding -> chapters, concepts, keyframes.
...
- Video understanding for chapters, concepts, and timestamps.
- Structured outputs (JSON schema) for questions and blueprints.
- File Search for grounding against slides, syllabus, PDFs.
- Code Execution to verify numericals and rubric checks.
- Interactions API for long-running, resumable jobs with progress.
- Live API for oral-viva mode (stretch).
- Deep Research for syllabus and past-paper blueprint (stretch).
- Computer-use for resource collection (stretch, opt-in).
...
A clear separation of backend services and frontend interface will ensure maintainability and scalability. Below is the proposed file and directory structure for VeriLearn Exam Pack:
    ...
    • backend/ – Python backend service (API and processing logic).
        ...
        ◦ gemini_client.py – Wrapper for calling Gemini models (3 Pro, 3 Flash, etc.) with configured tools.
...
API Contract: generate_notes(lecture: VideoInfo) -> NoteDocument. It returns a structured result (e.g., a dataclass containing the markdown content, word count, reading time, list of image references). It may produce multiple output formats (MD, HTML, PDF) depending on requested formats, but the core output is a Markdown of the notes.
    ...
    • Gemini Tool Use: If needed, the verification step can invoke a “Deep Research” capability – e.g., using the LLM’s ability to call a web search tool to confirm facts. (In the Pandoc PDF generator, a Google Search tool is optionally enabled to help fix Mermaid diagrams[6], demonstrating how an LLM can call external search when needed. We can leverage a similar approach for fact-checking important points by enabling a search function for the model in verification mode.)
API Contract: Exposes methods like verify_notes(note_doc: NoteDocument) -> VerifiedNoteDocument (which might annotate or adjust the content to ensure citations) and verify_answer(question, answer) -> bool/confidence (to be used when a user submits an answer, cross-checking if the answer is correct and well-supported by evidence).
    ...
    • Gemini Model Usage: For live conversation, we would use the Gemini Live API (a streaming model interface) to handle back-and-forth dialogue. The model can maintain context over the conversation, ask follow-up questions, or switch to explanation mode if the user is struggling. The “Live API for oral quiz” essentially means using a model endpoint that supports real-time, session-based interaction (Gemini 3 Pro in chat mode, or a specialized conversation agent).
    • Assist Mode (Computer Use): VeriCoach can also have an assist mode where the AI helps the student explore a topic deeper. In this mode, the model might utilize tool usage for demonstration – for example, executing a small code snippet mentioned in the lecture, or drawing a diagram (using Mermaid or other tools) to illustrate a concept. The mention of “Computer Use for Assist mode” refers to allowing the model to perform actions like running code, searching the web, or browsing files to assist in teaching. This is similar in spirit to OpenAI’s Code Interpreter or tool-using agents, providing a more hands-on learning help. For instance, if a lecture covers a math concept, the assist mode could have the model generate additional examples and verify them by actually computing results (using a Python tool).
...
API Contract: Endpoints could include: - POST /start-exam with parameters (like which course or playlist, number of questions) -> returns an exam_id and the list of questions (with IDs). - POST /submit-answer – (or one per question) submit an answer to a question. Alternatively, submit all at once at end. - POST /finish-exam – finalize the exam attempt and get results. However, a simpler design: the frontend can retrieve the question list via GET /generate-pack or a similar call (since the pack includes the questions), and then use a single POST /submit-answer per question or a batch submission. We will detail endpoints in the API section. Internally, veriexam.py functions like grade_answers(exam_id, user_answers) -> ResultReport would perform the grading using VeriBank’s answer key and possibly VeriVerify for free text answers.
    ...
    • If Mermaid diagrams are included in the markdown (the note generator might include diagrams using Mermaid syntax for class structures or processes[7]), the PDF generation should handle them. In our design, if a Mermaid diagram fails to render via pandoc, the module can use the Gemini model (with the Google search tool enabled) to attempt a fix[8][9]. This is an innovative use of the LLM: the fix_mermaid flag triggers the model to analyze the Mermaid code error and correct it, possibly searching online for syntax help. We include this to maximize our PDF output quality.
...
The provided scripts video_study_generator.py and pandoc_pdf_generator.py are prototypical implementations of key features. We will integrate and refactor their capabilities into the modules above:
    ...
    • We will embed this logic into VeriExports.generate_pdf(). It will call Pandoc via a subprocess similarly. If use_mermaid is enabled and Pandoc fails on a diagram, our module will catch the error, and then utilize the Gemini API (likely the same gemini_client but possibly a different prompt) to attempt an automatic fix[8][9]. This might involve prompting the model with the Mermaid code and error message to get a corrected diagram syntax. The script even allows enabling/disabling an internet search tool for the model during this fix[6] – we will include that as an option (enable_google_search) to let the model search for Mermaid documentation if needed. This kind of tool usage shows how Gemini can be used beyond just text generation, veering into code debugging territory, which is a strong technical execution point.
...
Gemini API Feature Mapping: We will leverage specific features of the Gemini API for corresponding functionalities: - Gemini 3 Pro – used for complex, blueprint-style tasks such as generating a comprehensive set of lecture notes or a week summary. Its stronger reasoning ability and larger context window (if available) will handle synthesizing long transcripts and ensuring the output is coherent and well-structured. For instance, when creating an overall course summary or cross-lecture insights, 3 Pro will be called because it can strategize and connect ideas deeply[16]. - Gemini 3 Flash – used for faster, focused tasks like classification, extraction, or quick verification. For example, verifying whether a particular detail is present in the transcript, or classifying the difficulty level of a question, can be done with the lightweight Flash model (which is ~3x faster and cheaper[17]). Also, during live quiz interactions, Flash might handle quick turns if high reasoning isn't needed, to reduce latency. - Gemini “Live” (Streaming API) – used in VeriCoach for oral quiz or interactive chat. A streaming model allows sending user utterances and getting partial responses in real-time, which is ideal for a conversational quiz where the model might ask a question and then react to the user's answer immediately. We'll utilize this for a smooth tutoring experience. - Tool Use (Function Calling) – We register tools like take_screenshot and possibly a google_search function with the Gemini model. The take_screenshot tool is mapped to our screenshot utility; the model will be prompted about this function availability (as done in the script) so it knows it can request a screenshot at a certain timestamp[2]. Similarly, a search_web(query) function could be exposed in verification or mermaid-fixing prompts, enabling Deep Research capability where the model explicitly asks to search for something (e.g., “search_web: 'Mermaid syntax loop node'”). The Gemini API (especially if using Google’s Vertex AI with tools) will handle returning function call requests which our code executes, then feed results back to the model. This mapping of API features to our tool flow is crucial in achieving evidence-backed results and rich outputs: - Deep Research – invoked in VeriVerify or VeriCoach when an external reference is needed. The model may call search_web and then incorporate results into its answer, which helps ensure factual accuracy and enriches the explanations with real data[6] (similar concept applied to diagram fixing). - Structured Output – we instruct the model to output certain parts in JSON or specific formats when needed. For example, the screenshot recommendations might be requested in JSON format within the model’s answer (the script looks for a JSON array in the response text[18]). Also, question generation might be prompted such that the model returns a JSON list of questions with fields (so we can easily parse and store them). Using the model’s capability to follow a format or using few-shot examples for JSON formatting will make parsing more robust. - Multi-turn Reasoning – for complex tasks like verifying content or generating a full week summary, we might allow the model to do chain-of-thought internally or even multi-turn interactions with tools. However, given time constraints, our primary approach is single-call per major output, enhanced with function calling for tools.
Tool Usage Flow (Gemini Agent and Screenshot Capture)
To clarify how the tools and model interact in the generation pipeline, here’s the flow: 1. Transcript & Prompt Preparation: Once VeriMap provides the video URL and title, and we have a transcript or ability to stream the video, VeriNotes formulates a prompt for Gemini. The prompt includes detailed instructions (derived from video_study_generator prompts) on how to output the notes, the desired sections, tone, etc. It also informs the model about available functions. For example: "You have access to a screenshot function that can capture video frames at specific timestamps..."[2] and guidelines like "Use this function ONLY if absolutely necessary and only after finishing main content..."[19]. 2. LLM Generation with Function Calls: We call the Gemini model (either 3 Pro or a specified model) with the prompt and a list of enabled tools (take_screenshot, possibly search_web). The model begins generating the notes. When it decides a screenshot would be helpful (say it’s describing a complex diagram from the video), it emits a function call instead of plain text. For example, it might output a JSON like: {"name": "take_screenshot", "args": {"timestamp": "00:15:30", "description": "Neural network architecture diagram"}}. Our gemini_client.py captures this call. 3. Screenshot Capture: The backend receives the function call, and screenshot_tool.py executes it – using cv2 to open the video file/stream, seek to 15:30, and save an image. The tool returns a unique reference ID or confirmation (e.g., “Screenshot reference: 12345”). This reference ID is passed back into the model’s generation. The model then continues its output, possibly inserting a placeholder like “(see diagram at 00:15:30 – refer to Screenshot 12345)”. 4. Completion and Post-processing: Once the model finishes, VeriNotes processes the output text. It looks for any "Screenshot reference: XYZ" tags in the text and replaces them with actual markdown image links using the data in screenshot_storage (filename and description)[4]. The end result is the markdown note containing sections, text, and images embedded in a Visual References section or inline (depending on how we format it; the current plan is to list all visuals under a heading for clarity). 5. Verification Pass (optional): VeriVerify can at this point scan the notes and perhaps call the model again (or use heuristic) to ensure that any claims are accompanied by either a timestamp or reference. If any section lacks support, it could append a footnote like “source: Lecture video at 07:20” or even query the transcript to find the relevant time. This step ensures the evidence gating: content is tied to source. If using the search tool, the model could also fetch an external reference for any fact that wasn't directly in the video, but typically the video transcript should be the primary source of truth. 6. Question Generation: After notes are finalized, VeriBank will create questions. It might call the model with a prompt like: “Generate X quiz questions covering the above content, in JSON with fields: question, options, answer, explanation, and reference (timestamp or section).” The model (probably using the faster Gemini model here as context is smaller – it can take the notes as input) returns the questions and answers in a structured form. VeriBank parses it and stores questions (without answers shown) for the user. 7. Packaging: Finally, VeriExports can be called to generate a PDF or other formats if requested. Meanwhile, VeriVault saves all outputs (notes, images, questions, answers) under a unique ID (the pack ID, often derived from the playlist name or a hash of the URL).
...
The VeriLearn Exam Pack system follows a service-oriented architecture with a clear separation between frontend, backend services, and external AI/Storage components. At a high level, the system comprises:
    ...
    • Gemini 3 Pro service – used for heavy tasks (notes generation, complex reasoning).
    • Gemini 3 Flash service – used for fast tasks (verifications, simple Q&A generation).
    • Gemini Live/Chat service – used for interactive sessions.
...
↘︎ Function Tools (Screenshot, Search)
...
- Client Coach (WebSocket) ↔︎ Backend ↔︎ Gemini Live Session (with possible tool access for complex queries).
...
We have introduced the modules in the implementation plan; here we detail how they interconnect, effectively creating a component diagram:
    ...
    • Transcript Extractor: may be called at the start to get textual content of video (unless Gemini model can directly process audio/video content, which the name "native video understanding" in the code suggests it might[25]. If Gemini has multi-modal input, we could feed the video or audio directly; if not, we get the transcript text ourselves).
...
Given the variety of tasks, choosing the right model for the job is critical for both performance and quality. Our strategy:
    • Gemini 3 Pro – Use for tasks requiring complex reasoning, long-form generation, or handling large context:
    • Notes Generation: The lecture transcripts and the requirement to produce structured, pedagogically-sound notes is complex. 3 Pro’s advanced reasoning will help it follow the format strictly and include all key points. It also might better handle tool usage decisions (like deciding which visuals are important enough for a screenshot).
    • Weekly/Course Summaries: Summarizing multiple lectures and threading concepts together is a strategic task suited to 3 Pro.
    ...
    • Gemini 3 Flash – Use for tasks where speed is important and context is limited:
    • Question Generation: After each lecture, generating a set of questions is relatively contained (the model just saw the lecture notes or transcript). We prefer Flash here to save time and cost, as it can likely produce good questions quickly[17]. If the quality is not as high, we can always verify or prompt it with examples.
    • Verification and Classification: Checking if an answer is correct (a relatively short text comparison) or deciding which sections of text to cite are quick tasks – Flash’s faster throughput is beneficial. For example, if verifying a factual statement, Flash can scan the transcript for a matching phrase faster (especially if we chunk and parallelize).
    • Chat Q&A (basic): If the coach is answering a straightforward factual question, Flash can be used with the notes context to give a quick answer. This provides a more responsive feel. We might reserve Pro for when the user explicitly asks for a deep explanation or if Flash's answer is found lacking (a potential dynamic fallback: try Flash first, if confidence low or user still confused, escalate to Pro).
    • Gemini Live (Streaming) – Use for interactive dialogue:
    • This is not a different model per se, but a mode of usage. We will use the streaming endpoint of whichever model fits. Likely, for conversation, we can start with Flash (for cost and speed), but ensure our prompts for the coach are designed so that it can handle multi-turn. If we find Flash isn't maintaining context well or giving superficial answers, we could use Pro in streaming mode for the coach despite latency, especially if answers are long. Perhaps a hybrid: use Flash for asking questions (which are short prompts from the AI) and use Pro when giving detailed feedback or explanations.
    • Additionally, the streaming is crucial for voice interface – we can start speaking the answer as it's generated (if TTS is used, though aligning that might be advanced).
    • Toolformer/Computer Use (Gemini with tool calls):
    • We specifically plan to use the model's ability to call external functions. For screenshot and search, both Pro and Flash can be enabled with these. However, we might choose to only enable certain tools with certain models. For example, when generating notes with Pro, enable take_screenshot function. When doing verification with Flash, enable search_web.
    • The "Computer Use for Assist mode" suggests if there's a scenario where the model might need to perform more complex tool-using (like writing and executing code, or performing calculations), we might integrate a Python execution tool. Gemini (especially a code-capable version, if available) could then do something like write a short snippet to demonstrate something. This is an aspirational part of the design for maximum innovation points. Implementation would involve something like an eval function sandboxed, registered as run_code tool. We mention this as a possible extension where the AI could say "Let's plot this function" and then call a tool to generate a graph or so.
To summarize, Gemini 3 Pro is our “thinking cap” and Gemini 3 Flash is our “workhorse sprinter”. We will calibrate prompts and the number of examples to compensate for Flash’s lighter reasoning when needed (e.g., providing structured prompt so it just fills in blanks). We also keep the door open to adjust this if usage limits or model performance dictate (like if 3 Pro has low rate limits, we might do more with Flash). The architecture allows easy switching since gemini_client will have model name and we can pass a parameter to each module about which model to use.
...
A core value proposition of VeriLearn is that all generated content is verifiable and trusted. The system is architected to enforce evidence-backed output at multiple levels:
    ...
    • Citation for External Info: If our model goes beyond the video content (perhaps in Further Reading sections or additional context the user asks), we instruct it to provide a proper citation (URL or title of source). For example, if the Further Reading suggests "Read the paper by Smith et al. 2023 on this topic", we want either a link or enough info to find it. If the search_web tool is used, it can fetch an actual URL that we can cite in the notes. The PDF export or HTML could have footnotes or reference list compiled.
...
The system should gracefully handle transient errors. Some possible errors and our approach:
    ...
    • Or use the thinking_level="low" for those to encourage less verbose output if needed.
    ...
    • E.g., if we use the search_web tool, ensure it cannot be misused (the model could try to search something malicious or irrelevant, we might restrict queries to certain domains or number of results).
